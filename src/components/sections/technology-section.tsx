...
export function TechnologySection() {
  ...
  useEffect(() => {
    // Fetch items (still from features as they are grouped there)
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/cms/features`, {
      ...
      .catch(err => console.error("Error fetching pillar cards:", err));

    // Fetch section header info directly from pillars_section
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/cms/pillars_section`, {
      headers: { 'Accept': 'application/json' }
    })
      .then(res => res.json())
      .then(json => {
      ...
      .catch(err => console.error("Error fetching pillars section data:", err));
  }, []);
  ...
  const displayTitle = contentBlock?.title || "Built for Legacy. Designed for Return.";
  const displayLabel = contentBlock?.tagline || "The Experience";

  const displayFeatures = features.length > 0 ? [...features].sort((a, b) => (a.sortOrder ?? a.sort_order ?? 0) - (b.sortOrder ?? b.sort_order ?? 0)) : [
    {
      title: "History",
  ...
  return (
    <section id="experience" className="bg-background">
      ...
      <div className="px-6 py-20 text-center md:px-12 md:py-28 lg:px-20 lg:py-32 lg:pb-20">
        <p className="text-xs uppercase tracking-widest text-muted-foreground font-semibold mb-4">{displayLabel}</p>
